...
def v(self):
  #Devuelve TAS
  if not self.es_spd_std: # Velocidad mï¿œima manteniedo IAS
  ...
  if self.alt<=inicio_app: # Velocidad de aproximación
    return self.spd_app
  elif self.alt<=trans_tma: # Transiciï¿œ entre vel aprox y tma
    p=(self.alt-inicio_app)/(trans_tma - inicio_app)
    return self.spd_app*(1.-p)+self.spd_tma*p
  elif self.alt<=vel_tma: # Transiciï¿œ entre ruta y tma
...
def f_vert(self):
  # Devuelve factor correcciï¿œ de velocidad vertical
...
def get_hdg_obj(self,deriva,t):
  # Da el rumbo objetivo en funciï¿œ de la demanda
  if self.to_do == 'fpr': # Mantener plan de vuelo
    self.pto=self.route[0][0] #Punto al que se dirige con correcciï¿œ de deriva
    self.vect=rp(r(self.pto,self.pos))
    # Correciï¿œ de deriva
...
# self.to_do_aux = [[coord,nombre,estim],derrota_acerc, tiempo_alej,Tiempo en alejam. = 0.0,Esta en espera=False/True,giro I=-1.0/D=+1.0]
    if not self.to_do_aux[4] and self.to_do_aux[0] in self.route: # An no ha llegado a la espera, sigue volando en ruta
      ...
      # Correciï¿œ de deriva
      return self.vect[1] - deriva
    else: # Estï¿œdentro de la espera, entramos bucle de la espera
      self.to_do_aux[4] = True
      if not self.to_do_aux[0] in self.route: # El fijo principal debe estar en la ruta. Si no estï¿œ se pone
        self.route.insert(0,self.to_do_aux[0])
      self.vect=rp((2.0*self.ground_spd,self.track)) # Con esta operaciï¿œ no nos borra el punto de la espera
      if len(self.to_do_aux)== 6: # Vamos a definir el rumbo objetivo, aï¿œdiï¿œdolo al final
        ...
        if aux > -60.0 and aux < 120.0: # Entrada directa
          r_obj = (self.to_do_aux[1] + 180.0  - deriva) %360. # Rumbo de alejamiento (con correcciï¿œ de deriva)
        else:
          r_obj = -((self.to_do_aux[1] + 180.0  -30.0 * self.to_do_aux[5] - deriva) %360.) # Rumbo de alejamiento (con correcciï¿œ de deriva)
      ...
      if self.to_do_aux[3] == 0.0 or self.to_do_aux[3] == -10.0: # Estï¿œen el viraje hacia el tramo de alejamiento
    ...
    if rdl<180.0 and rdl_actual>rdl+180.0:
      ...
      rdl_actual=rdl_actual+360.0
    ang_aux=rdl - rdl_actual #  Positivo, el radial estï¿œa la izquierda de posición actual
    (rdlx,rdly)=pr((1.0,self.to_do_aux[1]))
    dist_perp = abs(rx * rdly - ry * rdlx)
    if dist_perp < 0.1: # Consideramos que estï¿œen el radial
      self.vect=rp((2.0*self.ground_spd,self.track))
...
#       return (self.to_do_aux[1] - deriva - 45.0 * sgn(ang_aux))%360.0
    ...
    if len (self.route) > 1: # An no estï¿œen el localizador, tocamos solamente la altitud y como plan de vuelo
      if self._map and [xy_llz,'_LLZ',''] not in self.route: # Ya estï¿œfrustrando
    ...
    if len(self.route) == 1: # Interceptar localizador y senda de planeo
      if self._map and [xy_llz,'_LLZ',''] not in self.route: # Ya estï¿œfrustrando hacia el ltimo punto, asimilamos a plan de vuelo normal
        self.to_do = 'fpr'
        self.app_auth = False
        self.app_fix = ''
        self._map = False
        ...
        derrota = rp((rx,ry))[1]
        if abs(dist_thr) < 0.50: # Aviï¿œ aterrizado
        ...
        if dist_perp < 0.1: # Consideramos que estï¿œen el radial
          ...
          if self.int_loc:
            ...
            if rdl<180.0 and rdl_actual>rdl+180.0:
              ...
              rdl_actual=rdl_actual+360.0
            ang_aux2=rdl - rdl_actual #  Positivo, el radial estï¿œa la izquierda de posición actual
...
class Airplane:
  ...
  def __init__(self):
    ...
    self.hist=[] #Histï¿œico ltimos 5 puntos
    self.hist_t=0.0 # Tiempo del ltimo punto
    self.hdg=400.0 # ï¿œtimo rumbo calculado. Este valor es para saber la primera vez
    ...
    self.turn=3.0*60.*60. #Los mï¿œimos grados por segundo que vira el avión
  ...
  def next(self,t):
    ...
    # Iteraciï¿œ para encontrar la posición
...
#             self.to_do_aux = app
            ...
            self.t = t
            self.route.pop(0)
          elif self.route[0][1] in aeropuertos: # Si el ltimo punto estï¿œen la lista de aeropuertos, orbita sobre ï¿œ
    ...
    # Recï¿œculo del histï¿œico cada 5 segundos
    step=5./60./60.
  ...
  def tas(h):
    #Devuelve TAS en funciï¿œ de la altitud      
    if h>250:
  ...
  def se_debe_imprimir(self,t):
    # Definimos cuï¿œto tiempo antes nos sale la ficha y el tiempo de permanencia del mensaje
    prevision=10./60.
    permanece=2./60.
                        ...
                        if pto == point_name:
                          ...
                          for i in range(len(self.route)):
                            if self.route[i][1] == pto.upper():
                              aux = self.route[i:]
                          # Si no estï¿œen la ruta, insertamos el punto como n 1
                          if aux == None:
...
def vac(a,b):
  # Comprueba si es una violaciï¿œ de la separaciï¿œ
  if not (a.esta_asumido and b.esta_asumido):
    return False
  minvert=9.00 #MINIMA DE SEPARACIï¿œ VERTICAL
  t=max(a.t,b.t)
...
def pac(a,b):
  # Aviso de pï¿œdida de separaciï¿œ
  if not (a.esta_asumido and b.esta_asumido):
    return False
  aviso=1.0/60.  # PARï¿œETRO DE TIEMPO EN EL CUAL SE PREVï¿œQUE HAY VAC (1 min)
