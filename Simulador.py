...
def end_def_lad(e, canvas = w):
  # Fin de creaciï¿œ de un LAD con destino un AVO
...
#     # ... salvo que se haya tirado un LAD entre un avión y ï¿œ mismo!
#     if lad_origen == self:
#           print "LAD entre un avión y ï¿œ mismo. Ignorando..."
#           return
    canvas.delete('lad_defined')
    canvas.unbind('<Motion>')
    definiendo_lad = 0
...
# Tratamiento del botï¿œ izquierdo con el plan de vuelo sobre el plot
...
def se_cortan (label_modif,i,j):
  # Devuelve si las etiquetas estï¿œ separadas entre los aviones i y j de ejercicio
...
#       cond1 = oa_x_vperp * ob_x_vperp # Negativo si cada uno estï¿œa un lado
...
def timer():
  ...
  refresco=5.
  # Si el reloj estï¿œparado actualizamos t0 para ajustarque no corra el tiempo y no actualizamos.
...
#     return
  ...
  if tlocal(t0)-last_update<refresco:
    ...
    # Mover los aviones con auto-separaciï¿œ
    for a in ejercicio:
        ...
        while (intersectan_girado > 0) and (cuenta[0] < 8) and (tlocal(t0)-crono_ini)/fact_t < 0.5:
          for k in range(len(moviendo)-1,-1,-1):
            if cuenta[k]<8:
              ...
              cuenta[k] = 0 
          # Comprobamos si estï¿œ separados todos entre ellos
...
#           print 'cuenta: ',cuenta, intersectan_girado
        # En caso de que haya conflicto, escogemos el giro con menos intersecciones
          if intersectan_girado < menos_inter:
            ...
            cuenta_menos_inter = cuenta
          # Comprobamos que no estemos afectando a ningn otro avión con el reciï¿œ girado. En caso contrario, se añade
...
def define_holding():
  ...
  if sel == None:
    ...
    def set_holding(e=None):
            ...
            auxiliar = ''
            # Si la espera estï¿œpublicada, los datos de la espera
...
def b_execute_map():
    ...
    if sel == None or not sel.app_auth:
      win = Frame(w)
      txt_ruta0 = Label (win,text='Asignar ejecuciï¿œ aproximación frustrada')
      txt_ruta = Label (win,text='NO HAY NINGUN VUELO SELECCIONADO ',fg='red')
