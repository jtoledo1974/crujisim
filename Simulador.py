...
# Definici� de LAD's en el canvas
class fix_point:
...
def end_def_lad(e, canvas = w):
  # Fin de creaci� de un LAD con destino un AVO
  global definiendo_lad, lad_origen
  if definiendo_lad == 0:
    # No se estaba definiendo un LAD. Ignorar evento
    return
  elif definiendo_lad == 1:
    # Se estaba definiendo un LAD. Terminar definici�...
#     # ... salvo que se haya tirado un LAD entre un avi� y � mismo!
#     if lad_origen == self:
#           print "LAD entre un avi� y � mismo. Ignorando..."
#           return
    canvas.delete('lad_defined')
    canvas.unbind('<Motion>')
    definiendo_lad = 0
...
# Tratamiento del bot� izquierdo con el plan de vuelo sobre el plot
    ...
    if t>t_etd and button['bg'] <> 'green':
      def dep_avo(x=manual.winfo_x()+button.winfo_x()+button.winfo_width(),y=manual.winfo_y()+button.winfo_y(),button_avo=button,callsign=callsign,sid=sid,etd=etd):
        ...
        def despegue_avo(callsign=callsign,sid=sid):
          ...
          # Ecoger el avi� a despegar
          for a in ejercicio:
            if a.get_callsign() == callsign:
              break
          # Tomamos la sid escogida y la asignada autom�icamente
          ...
          if sid.upper() not in sid_final.upper():
            ...
            for i in range(len(a.route)):
              if a.route[i][1] in sid.upper():
                ...
                break
          # Ahora se depega el avi� y se elimina de la lista
...
def se_cortan (label_modif,i,j):
  ...
  if ejercicio[i].is_flying():
    ...
    # Comprobamos las cuatro esquinas del avi� j y que no se corten los soportes de etiquetas, as�como ningn plot
...
#     return
  ...
  if tlocal(t0)-last_update<refresco:
    ...
    # Mover los aviones con auto-separaci�
    for a in ejercicio:
...
#               print 'A�diendo ',ejercicio[j].get_callsign()
              moviendo.append(j)
              cuenta.append(0)
              giro_min.append(0)
...
#           print 'cuenta: ',cuenta, intersectan_girado
        # En caso de que haya conflicto, escogemos el giro con menos intersecciones
          if intersectan_girado < menos_inter:
            ...
            cuenta_menos_inter = cuenta
          # Comprobamos que no estemos afectando a ningn otro avi� con el reci� girado. En caso contrario, se a�de
    ...
    def set_holding(e=None):
            ...
            if auxiliar == '':
              for i in range(len(sel.route)):
                [a,b,c] = sel.route[i]
                if b == fijo:
                  if i == 0: # La espera se inicia en el siguiente punto del avi�
                    auxi = sel.pos
                  else:
            ...
            if not error:
              ...
              # Cancelar posible autorizaci� de aproximaci�
              cancel_app_auth(sel)
...
def hdg_after_fix():
    ...
    if sel == None:
      win = Frame(w)
      txt_ruta0 = Label (win,text='Rumbo despu� de fijo')
      txt_ruta = Label (win,text='NO HAY NINGUN VUELO SELECCIONADO ',fg='red')
      but_acept = Button(win, text="Aceptar")
      txt_ruta0.pack(side=TOP)
      txt_ruta.pack(side=LEFT)
      ...
      if vent_ident_procs != None:
        ...
        vent_ident_procs = None
        win = Frame(w)
      title = Label(win, text = 'Rumbo despu� de fijo: '+sel.get_callsign())
...
def b_execute_map():
    ...
    if sel == None or not sel.app_auth:
      win = Frame(w)
      txt_ruta0 = Label (win,text='Asignar ejecuci� aproximaci� frustrada')
      txt_ruta = Label (win,text='NO HAY NINGUN VUELO SELECCIONADO ',fg='red')
      txt2_ruta = Label (win,text='O EL VUELO NO EST�AUTORIZADO APP ',fg='red')
      but_acept = Button(win, text="Aceptar")
      txt_ruta0.pack(side=TOP)
      txt_ruta.pack(side=LEFT)
...
def b_int_llz():
    ...
    if sel == None:
      ...
      if sel.fijo_app == 'N/A':
        ...
        txt_ruta = Label (win,text='VUELO SIN IAF, a�da ruta al IAF y reintente ',fg='red')
        but_acept = Button(win, text="Aceptar")
        txt_ruta0.pack(side=LEFT)
        txt_ruta.pack(side=LEFT)
        but_acept.pack(side=LEFT)
      ...
      def int_llz(e=None):
              ...
              sel.int_loc = True
              (puntos_alt,llz,puntos_map) = proc_app[sel.fijo_app]
              # En este paso se desciende el tr�ico y se a�den los puntos
...
def b_auth_approach():
    ...
    if sel == None:
      win = Frame(w)
      txt_ruta0 = Label (win,text='Autorizar a aproximaci�')
      txt_ruta = Label (win,text='NO HAY NINGUN VUELO SELECCIONADO ',fg='red')
      but_acept = Button(win, text="Aceptar")
      txt_ruta0.pack(side=TOP)
      txt_ruta.pack(side=LEFT)
      ...
      def auth_app(e=None,avo=sel):
              ...
              print "Autorizado aproximaci�: ", avo.route
              win_identifier=None
              close_win()
