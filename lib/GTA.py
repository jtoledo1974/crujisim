...
class GTA(object):
    ...
    def change_rwyInUse(self, ad_designator, rwy_direction_desig):
        """Modifies the rwy in use in for the given airport, and
        changes the SID and STAR procedures for the relevant aircraft"""
        # TODO this has to be updated to change airport configuration
        # Airports do have more than one runway available.
        try:
            ad = AIS.aerodromes[ad_designator]
            rwy_direction = [rwy for rwy in ad.runwayDirections
                             if rwy.designator == rwy_direction_desig][0]
        except Exception:
