...
def load_exercises(path, reload=False):
    ...
    for f in os.listdir(d):
        if f[-4:] != ".eje" and f != MAPPING_FILE_NAME:
            continue
        date = os.stat(os.path.join(d, f))[ST_MTIME]
        if date > recent:
            recent = date

    # Find whether a cache file exists or if it's older than the most recent
    # exercise
    cache = os.path.join(d, ".cache")
    if not reload and os.access(cache, os.F_OK | os.R_OK) \
        ...
        try:
            ...
            if version == CACHE_VERSION:
                ...
                logging.info("Cache file " + cache + " is version " +
                             str(version) +
                             ", different than currently supported version " +
                             str(CACHE_VERSION))
            c.close()
        except:
            logging.warning("Unable to load cache file: " + cache)
    else:
        logging.info("Cache file " + cache +
                     " not found, not readable or stale")
    ...
    for f in [f for f in os.listdir(d) if f[-4:] == ".eje"]:
        ...
        try:
            for (course, phase, day, pass_no, shift) \
                    in mapping.exercises[(e.da, e.usu, e.ejer)]:
                ne = e.copy()
...
class Exercise(object):
    ...
    def load(self, file):
        ...
        try:
            self.da = exc.getint('datos', 'da')
        except:
            self.da = None
        try:
            self.usu = exc.getint('datos', 'usu')
        except:
            self.usu = None
        try:
            self.ejer = exc.getint('datos', 'ejer')
        except:
            self.ejer = None
        try:
            self.course = exc.getint('datos', 'course')
        except:
            self.course = None
        try:
            self.phase = exc.getint('datos', 'phase')
        except:
            self.phase = None
        try:
            self.day = exc.getint('datos', 'day')
        except:
            self.day = None
        try:
            self.pass_no = exc.getint('datos', 'pass_no')
        except:
            self.pass_no = None
        try:
            self.shift = exc.get('datos', 'shift')
        except:
            self.shift = ""
        try:
            (self.wind_azimuth, self.wind_knots) = [
                int(x) for x in exc.get('datos', 'viento').split(",")]
        except:
            self.wind_azimuth, self.wind_knots = 0, 0
        ...
        try:
            self.oldcomment = exc.get('datos', 'comentario')
        except:
            self.oldcomment = os.path.basename(file)
        # If the file doesn't have an actual new comment attr yet, use the old
        # one.
        try:
            self.comment = exc.get('datos', 'comment')
        except:
            self.comment = self.oldcomment
            ...
            try:
                for attrib, index in m.items():
                    if attrib in ("da", "usu", "ejer"):
                        if self.__dict__[attrib] is None:
                            self.__dict__[attrib] = int(match.group(index))
                    else:
    ...
    def load_flights(self, exc):
        ...
        for flightopt in flightopts:
            try:
                self.flights[i] = Flight(
                    flightopt, exc.get('vuelos', flightopt))
                i += 1
            except:
                logging.warning("Unable to read flight " +
                                str(flightopt).upper() + " from " + str(file))
    ...
    def save(self, file):
        ...
        exc.set('datos', 'viento', "%03d" %
                self.wind_azimuth + "," + "%02d" % self.wind_knots)

        try:
            exc.add_section('vuelos')
        except:
            pass
        ...
        try:
            for (sa, sv) in self.__dict__.items():
                ...
                if getattr(other, sa) != sv:
                    logging.debug(sa + " differs when comparing exercises ( " + str(sv) + " " + str(type(
                        sv)) + " != " + str(getattr(other, sa)) + " " + str(type(getattr(other, sa))) + " )")
                    return False
            for f in self.flights.values():
                f2 = [f2 for f2 in other.flights.values() if f2.callsign == f.callsign][
                    0]
                if f != f2:
                    return False
            for f2 in other.flights.values():
                f = [f for f in self.flights.values() if f.callsign ==
                     f2.callsign][0]
                if f != f2:
                    return False
        except:
...
class Flight(object):
    ...
    def fix_eto_firstlevel_tas(self):
        ...
        for fix, i in zip(route, list(range(len(route) - 1))):
            ...
            if m:
                break
        try:
            # Make sure it's six chars long, and right pad with zeroes
            eto = m.group(1).ljust(6, "0")
            firstlevel = m.group(2).rjust(3, "0")
            tas = m.group(3).rjust(3, "0")
        except:
            logging.warning(
                "Unable to extract HFV field in '" + self._data + "'")
            raise
    ...
    def get_config_string(self):
        try:
            s = self.type + "," + self.wtc + "," + self.adep + "," + self.ades + \
                "," + "%03d" % (int(self.rfl)) + "," + "%03d" % (int(self.cfl))
            for f in self.route.split(","):
                s += "," + f
                if f == self.fix:
                    s += ",H" + self.eto + "F" + \
                        "%03d" % (int(self.firstlevel)) + \
                        "V" + "%03d" % (int(self.tas))
            return s
        except:
            logging.error(
                "Failure to create config string for " + str(self.callsign))
            raise
    ...
    def __setattr__(self, name, value):
        ...
        object.__setattr__(
            self, name, value)  # This actually sets the attributes
            ...
            for opt in mapping.options('Mappings'):
                try:
                    ...
                    for s in mapping.get('Mappings', opt).split(","):
                        try:
                            (course, phase, day, pass_no, shift) = (
                                int(s[1:3]), int(s[4:5]), int(s[6:8]), int(s[9:10]), s[11:12])
                            self.exercises[(da, u, e)].append(
                                (course, phase, day, pass_no, shift))
                        except:
                            logging.warning(
                                "Incorrect mapping format " + s + " in mapping file " + f)
                except:
                    logging.warning(
                        "Unable to read mapping for excercise " + opt + " in " + f)
        except:
            ...
            if (adep + ades) in orig_dest_list:
                ...
                for od in orig_dest_list:
                    if (adep == od[0:4]) or (ades == od[4:8]):
                        # But if either the adep or ades matched we get partial
                        # score
                        matches_orig_dest = 1
...
if __name__ == '__main__':
    # Exercise("../pasadas\APP-RadarBasico\21-phase-1-Día-01-M-TMA
    # Madrid-1.eje"
